...
namespace DoAnPhanMem.Controllers
{
    public class AdminController : Controller
    {
        ...
        public PartialViewResult DoanhThuDichVu()
        {
            ...
            var dichVuTheoThang = db.HOADON_DV
                .Where(x => x.HOADON.NGAY.Year == DateTime.Now.Year)
                .GroupBy(x => new { x.HOADON.NGAY.Month, x.DICHVU.TENDV })
                .Select(g => new
                {
                    Month = g.Key.Month,
                    Service = g.Key.TENDV,
                ...
                .Select(g => new
                {
                    ...
                    DichVu1 = g.Where(x => x.Service == "DichVu1").Sum(x => x.Count),
                    DichVu2 = g.Where(x => x.Service == "DichVu2").Sum(x => x.Count),
                    // Thêm các dịch vụ khác tương tự
                })
                .ToList();

            ViewBag.doanhThuDichVu = doanhThuDichVu;
            ViewBag.dichVuTheoThang = dichVuTheoThang;
        ...
        [HttpGet]
        public ActionResult LichSu(int pageIndex = 1, int pageSize = 10, DateTime? fromDate = null, DateTime? toDate = null, string searchTerm = "")
        {
            ...
            {
                data = data.Where(x => x.TENKH.Contains(searchTerm) || x.EMAIL.Contains(searchTerm));
            }
        ...
        [HttpGet]
        public ActionResult HoaDon(int pageIndex = 1, int pageSize = 10, DateTime? fromDate = null, DateTime? toDate = null, string searchTerm = "")
        {
            ...
            if (!string.IsNullOrEmpty(searchTerm))
            {
                data = data.Where(x => x.TENKH.Contains(searchTerm) || x.TENNV.Contains(searchTerm));
            }
